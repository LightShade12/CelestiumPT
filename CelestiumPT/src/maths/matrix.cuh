...
class Matrix4x4 {
public:
	__device__ Matrix4x4() : columns{ make_float4(1, 0, 0,0), make_float4(0, 1, 0,0),
		make_float4(0, 0, 1,0),make_float4(0,0,0,1) } {};

	__device__ Matrix4x4(float4 c1, float4 c2, float4 c3, float4 c4) {
		columns[0] = c1; columns[1] = c2; columns[2] = c3; columns[3] = c4;
	};

	__device__ float4& operator [] (size_t idx) {
		return columns[idx];
	}
	__device__ const float4& operator[](size_t idx) const {
		return columns[idx];
	}
	...
	__device__ Matrix4x4 operator*(const Matrix4x4& mat2) const {
	...
	__device__ Matrix4x4 transpose() const {
		return Matrix4x4(
	...
	__device__ static Matrix4x4 getIdentity() {
		return Matrix4x4(
			make_float4(1, 0, 0, 0),
			make_float4(0, 1, 0, 0),
			make_float4(0, 0, 1, 0),
	...
	__device__ static Matrix4x4 getZero() {
		return Matrix4x4(
			make_float4(0, 0, 0, 0),
			make_float4(0, 0, 0, 0),
			make_float4(0, 0, 0, 0),
	...
	__device__ static void print_matrix(const Matrix4x4& mat) {
