...
//Traversal
__device__ void traverseBVH(const Ray& ray, const int root_node_idx, ShapeIntersection* closest_hitpayload, const SceneGeometry* scenedata) {
	...
	while (stackPtr > 0) {
		...
		else
		{
			for (int primIndiceIdx = stackTopNode->left_child_or_triangle_indices_start_idx;
				...
				int primIdx = scenedata->DeviceBVHTriangleIndicesBuffer[primIndiceIdx];
				primitive = &(scenedata->DeviceTrianglesBuffer[primIdx]);
...
//__device__ const int traverseBVH_raytest(const Ray& ray, const int root_node_idx, const SceneGeometry* scenedata) {
...
//	const BVHNode* stackTopNode = &(scenedata->DeviceBVHNodesBuffer[root_node_idx]);//is this in register?
//
//	ShortShapeIntersection workinghitpayload;
//	float hit1 = -1;
//	float hit2 = -1;
//	const Triangle* primitive = nullptr;
//
