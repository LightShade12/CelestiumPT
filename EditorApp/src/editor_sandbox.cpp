...
void EditorSandbox::initialise()
{
	...
	m_ModelImporter.loadGLTFfromFile("../models/cornell_box.glb", m_HostSceneHandle);//uses host API to add scene geo
	...
	if (!m_PhysCFG.enabled) {
		for (Mesh mesh : m_Meshes)
		{
			...
			if (mesh.host_mesh_handle.name == "Light") {
				translation.x = mesh.translation.x + (1.4 * sinf(glfwGetTime() * 2));
				translation.z = mesh.translation.x + (1.4 * cosf(glfwGetTime() * 2));
				updatemesh |= true;
			}
			...
			if (ImGui::BeginTabItem("Rendering")) {
				if (ImGui::CollapsingHeader("Debug")) {
					...
					ImGui::Combo("Renderer mode", (int*)&curent_renderview,
						"Composite\0Normals\0Positions\0GAS Debug\0UVs\0Barycentrics\0ObjectID\0LocalPosition\0Velocity\0Depth\0Albedo\0Variance\0Heatmap\0bbox heatmap\0sparse grads\0");
